...
for language in languages:
    ...
    destination_dir = os.path.join(
        home, f"dataset/machine/llama-3.1-8B/{language}_files"
    )
    ...
    for file_name in tqdm(
        os.listdir(source_dir), desc=f"Processing files for {lang_name}", unit="file"
    ):
        if file_name.endswith(".txt"):
            ...
            messages = language_settings[language]["prompt"](
                title, sentence, article_length
            )
    ...
    print(
        f"Article generation complete for {lang_name}. Files saved in 'dataset/machine/llama-3.1-8B/{language}_files'."
    )
